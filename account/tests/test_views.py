...
class RegisterViewTestCase(APITestCase):
    def setUp(self):
        ...
        # Create a client with csrf token access. 
        self.csrf_client = APIClient(enforce_csrf_checks=True)
    


    def _get_csrf_token(self):
      # Send a GET request to the csrf_cookie endpoint
    ...
    def test_get_csrf_token(self):
        # creating a GET request to 'csrf_cookie' url
        response = self.client.get(reverse('csrf_cookie'))

        # testing that, it has 'csrftoken' value or not
    ...
    def test_register_view_success(self):
        ...
        # creating a 'headers' data with 'X-CSRFToken' & 'Cookie'
        headers = {
            'X-CSRFToken': self.csrf_token, 
            'Cookie':'csrftoken='+self.csrf_token
        }
        
        # creating a POST request to the 'register' url
        # also using headers data for csrf protection
        response = self.csrf_client.post(
            url, data, format = 'json', headers = headers
        )


        # testing that, the registration process is successful as it provides 201 status code
        self.assertEqual(
            response.status_code, status.HTTP_201_CREATED
        )

        # also testing the use name and email address
    ...
    def test_register_view_invalid_email(self):
        ...
        # testing that, if the email is invalid, it provides 400 status code
    ...
    def test_register_view_duplicate_email(self):
        # creating an user with demo dat
        ...
        # testing that, if the email is duplicate, it provides 400 status code
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
# I have used there simple 'client' instead of 'csrf_client' as i am not going to use csrf validation for simplicity
...
class UserDetailViewTestCase(APITestCase):
    def setUp(self):
        ...
        # Make Authenticated by using 'force_authenticate' method
